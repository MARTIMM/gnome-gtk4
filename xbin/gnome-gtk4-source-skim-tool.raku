...
sub setup-names ( Str:D $base-name --> List ) {
  ...
  # $*base-sub-name has value from commandline or from $base-name which is e.g.
  # gtk_about_dialog. Lib class name becomes GtkAboutDialog.
  my @parts = $*base-sub-name.split(/<[_-]>/);
  $*lib-class-name = @parts>>.tc.join;

  # And Raku classname is then AboutDialog.
...
note "Files: $base-name, $include-file. $*lib-class-name, $*raku-class-name,  @parts.raku()";
  ...
  my @source-list = (
    "$source-root/gtk-4.6.3/gtk",
    "$source-root/gtk-4.6.3/gdk",
  );

  my Str ( $include-content, $source-content) = ( '', '');
  my Bool $file-found = False;

  # Walk through each of the paths
  for @source-list -> $path {
    # And search for the include file of the given base name
    # e.g. gtkaboutdialog.h
    if "$path/$include-file.h".IO.r {

      # If found, get the contents of .h and .c files
      $file-found = True;
      $include-content = "$path/$include-file.h".IO.slurp;
      $source-content = "$path/$include-file.c".IO.slurp
        if "$path/$include-file.c".IO.r;

      # Define native lib reference $*library. This must be noted in the
      # Gnome::N::NativeLib to refer to the system object library.
      # $*raku-lib-name is part of the full class name; Gnome::Gtk4::AboutDialog
      given $path {
        when / 'gtk-' <-[/]>+ '/gtk' / {
          $*library = '&gtk4-lib';
          $*raku-lib-name = 'Gtk4';
        ...
        # library is the same as for gtk in version 4
        when / 'gtk-' <-[/]>+ '/gdk' / {
          $*library = "&gtk4-lib";
          $*raku-lib-name = 'Gdk4';
        }
...
#note $*lib-class-name, ', ', $source-content ~~ m/ '/**' \s+ '*' \s+ $*lib-class-name ':' /;
...
#exit;
  ...
  my Str $short-description =
    primary-doc-changes(cleanup-source-doc($<short-doc>.Str));
  my Str $section-doc =
    primary-doc-changes(cleanup-source-doc($<long-doc>.Str));

  ( $section-doc, $short-description, '')
...
#note "$search-prop-doc, $search-prop-spec";
...
#        .*? ');'                            # till the spec ends
  ...
  my Str $first-prop = ($source-content ~~ m/
    '/**' \s+ '*' \s+ <alnum>+ ':' [<alnum> || '-']+ ':'
  / // '').Str;
  my Int $first-prop-index = $source-content.index($first-prop);

  # Every c-source will have its property definitions the same for each
  # of the defined properties. $try-method is used to select the way a property
  # is searched and found. It starts with the first. When it returns results,
  # it keeps that search method. If not it tries the second, etc.
  my Int $try-method = 1;

#note "property-search 1";
  ( $doc, $spec) = try-prop-search1( $source-content, $first-prop-index)
    if $try-method == 1;

  return ( $doc, $spec) if (?$doc and ?$spec);
  $try-method = 2;

#note "property-search 2";
  ( $doc, $spec) = try-prop-search2( $source-content, $first-prop-index)
    if $try-method == 2;
  return ( $doc, $spec) if (?$doc and ?$spec);
  $try-method = 3;

#note "property-search 3";
  # Keep this one at the last option. It checks for props without doc
  ( $doc, $spec) = try-prop-search-last( $source-content, $first-prop-index)
    if $try-method == 2;
  return ( $doc, $spec) if ?$spec;

#note "property-search - no result";
  ( '', '')
}

#-------------------------------------------------------------------------------
# Try to find a c-doc first followed by a g_param_spec_*()
sub try-prop-search1 ( $source-content is rw, $first-prop-index --> List ) {

#note $source-content ~~ m/'/**' \s+ '*' \s+ <alnum>+ ':' [<alnum> || '-']+ ':' \s+ '(' <-[\)]>+ ')' .*? '*/' \s* <alnum>+ \s* '=' \s* /;
#note "idx: $first-prop-index, ", $source-content.substr($first-prop-index);

  $source-content.substr($first-prop-index) ~~ m/
    $<property-doc> = [
      '/**'                               # start c-comment block
      \s+ '*' \s+ <alnum>+ ':'            # classname
      [<alnum> || '-']+ ':'               # property name
      \s+ '(' <-[\)]>+ ')'                # attributes part: not needed
...
#-------------------------------------------------------------------------------
# Try to find a c-doc first followed by a g_param_spec_*()
sub try-prop-search2 ( $source-content is rw, $first-prop-index --> List ) {
...
sub try-prop-search-last ( $source-content is rw, $first-prop-index --> List ) {
...
sub get-prop-doc ( Str $search-prop-doc is rw --> List ) {
  $search-prop-doc ~~ m/
    $<prop-spec> = [
      \s+ '*' \s+ $*lib-class-name ':'          # classname
      $<prop-name> = [ [<alnum> || '-']+ ] ':'  # property name
      \s+ '(' <-[\]]>+ ')'                      # attributes part: not needed
    ]
  /;

  my Str $prop-spec = ~($<prop-spec> // '');
  return ( '', '') unless ?$prop-spec;

  my Str $property-name = ~($<prop-name> // '');

  # Remove doc header which is a classname, property name and attributes
  $search-prop-doc ~~ s/ $prop-spec //;
...
#note "\nprimary-doc-changes 0: $text";
  ...
  podding-chapters($text);

  podding-signal($text);
#note "\nprimary-doc-changes signal: $text";

  podding-property($text);
...
#note "\nprimary-doc-changes vars: $text";
  ...
  modify-percent-types($text);
#note "\nprimary-doc-changes % types: $text";
}}

  podding-function($text);
#note "\nprimary-doc-changes function: $text";

#`{{
  adjust-image-path($text);
...
#-------------------------------------------------------------------------------
# change any # in =head. number of # is count of header
sub podding-chapters ( Str:D $text is rw ) {
  $text ~~ s:g/'######'/=head6/;
  $text ~~ s:g/'#####'/=head5/;
  $text ~~ s:g/'####'/=head4/;
  $text ~~ s:g/'###'/=head3/;
...
sub podding-signal ( Str:D $text is rw ) {
...
sub podding-property ( Str:D $text is rw ) {
...
sub podding-class ( Str:D $text is rw ) {

  $text ~~ s:g/ '[class@Gtk.' (<-[\]]>+) ']' /B<Gnome::Gtk4::$0>/;
#  $text ~~ s:g/ '[class@Gdk.' (<-[\]]>+) ']' /B<Gnome::Gdk4::$0>/;
}

#-------------------------------------------------------------------------------
# change any function() to C<function()>
sub podding-function ( Str:D $text is rw ) {

  my Str ( $spec, $class, $method);
  loop {
    $text ~~ m/ $<spec> = [
                  '[' [ method || vfunc ] '@Gtk.'
                  $<class> = [ <-[\.]>+ ] '.'
...
#note "spec: $<spec>.Str()";
    ...
    $class = ~($<class> // '');
#note "$class, $*raku-class-name";
    $class = $class eq $*raku-class-name ?? '' !! "Gnome::Gtk4::$class";
    $method = ~($<method> // '');
    $method ~~ s:g/ '_' /-/;

    # We can substitute this globally
    $text ~~ s:g/ $spec /C<$class.$method\(\)>/;
  }
}
